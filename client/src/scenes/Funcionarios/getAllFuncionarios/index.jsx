...
const columns = (handleDelete) => [
  ...
  },
  {
    ...
    renderCell: (params) => (
      <Link
        to={`/users/${params.row.userId}`} // Mudou de _id para userId
  ...
  },
  {
    ...
    width: 130,
    renderCell: (params) => {
      return params.row.role.rolesName || "N/A";
    },
  },
  {
    field: "delete",
...
const UserDataGrid = () => {
  ...
  const [signatureIdToDelete, setSignatureIdToDelete] = useState(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      setSnackbarMessage(
        "O Funcionário foi eliminado do sistema, juntamente com a sua assinatura e pagamentos associados."
      );
      setSnackbarOpen(true);
      refetch();
    } catch (err) {
      setSnackbarMessage(
        "Falha ao eliminar o Funcionário ou assinatura." +
  ...
  if (error)
    return <ErrorOverlay error={error} dataName={"Todos os funcionários"} />;
    ...
    <>
      <Box>
        <Header
          ...
          subtitle="Gerencie os funcionários do ginásio."
        />
        ...
        >
          ...
          <DialogContent>
            <DialogContentText id="alert-dialog-description">
              Ao eliminares este Funcionário vais também eliminar a sua
              assinatura. Esta ação é irreversível.
            </DialogContentText>
          </DialogContent>
          <DialogActions>
            <Button
